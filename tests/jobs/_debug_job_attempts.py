...
def _netbox_snapshot():
    ...
    device_type = DeviceType(
        manufacturer=manufacturer, model="WS-C2960X", slug="ws-c2960x"
    )
    return NetBoxInventorySnapshot(
        devices=(Device(name="switch-01", device_type=device_type, serial="ABC123"),)
    )
...
async def main() -> None:
    ...
    with session_factory() as db_session:
        ...
        print(
            "submitted",
            submission.job.id,
            submission.job.state.status,
            submission.job.state.attempts,
        )
