"""Provision the durable discovery-job cancellation permission for admins.
...
revision: str = "20260824_1300"
down_revision: str | None = "20260824_1200"
branch_labels: None = None
depends_on: None = None

_ADMIN_ROLE = "admin"
_PERMISSION = "discovery:job:cancel"
...
def provision_discovery_job_cancel_permission(connection: sa.Connection) -> None:
    """Idempotently provision only the global admin cancellation permission."""
    ...
    if permission_id is None:
        ...
        connection.execute(
            permissions.insert().values(
                id=permission_id,
                name=_PERMISSION,
                description="Administrative access: discovery:job:cancel",
                created_at=now,
                updated_at=now,
            )
        )
...
def upgrade() -> None:
    provision_discovery_job_cancel_permission(op.get_bind())
